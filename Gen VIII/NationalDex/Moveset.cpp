...
Moveset::Moveset(const vector<Attack>& ATTACK_DEX,
    ...
    unsigned int levelIndex = 0;  //PROC - The index in the level array
...
//Return the TM/TR Moveset as a string
QString Moveset::PrintTM() const{
    using namespace movesetGlobalConsts; //Using the column constants

    QString s;

    QTextStream out(&s);                      //OUT  - the output stream var

    QString tm;                           //PROC - the number of the TM
    ...
    if(!TMMoveset.empty()){
        ...
        out << endl << qSetFieldWidth(0) << LINE << endl;
    ...
    if(!lvlMoveset.empty()){
        ...
        for(auto it: lvlMoveset){
            ...
            out << qSetFieldWidth(LVL_COL)    << it.lvl               << " "
                << qSetFieldWidth(NAME_COL)   << QString::fromStdString(it.attack.name)	    << " "
                << qSetFieldWidth(TYPE_COL)   << QString::fromStdString(it.attack.type)	    << " "
                << qSetFieldWidth(CAT_COL)    << QString::fromStdString(it.attack.category) << " "
                << qSetFieldWidth(ATK_COL)    << it.attack.pwr	    << " "
                << qSetFieldWidth(ACC_COL)    << it.attack.acc	    << " "
                << qSetFieldWidth(PP_COL)     << it.attack.pp		    << " "
                << qSetFieldWidth(EFFECT_COL) << it.attack.effRate    << endl;
